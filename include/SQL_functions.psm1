...
#used to avoid 'Data would be truncated' SQL errors, since IDMS has small varchar name fields
function truncate-string {
    ...
    process {
        if ($String.Length -gt $Length) { $String.Substring(0, $Length) }
        else { $String }
    }
}

#escape single quotes in names (could be expanded to do more SQL cleanup)
...
<#
	.SYNOPSIS
		Runs a SQL query against the target SQL Server and database using the integrated credentials of the calling user account.

	.DESCRIPTION
		Uses the .NET SQLclient class to connect to a target SQL Server instance over TCP and execute the query supplied. The results are always returned as an array
        of objects, where each column in the result is a property of the object. 

		.OUTPUTS
		System.Object[]
	
	.NOTES
		Original Version 3/29/2010: Jeffrey B. Smith
		
		EDIT 4/2/2010: Added try/catch logic around ExecuteReader(), as this is where most SQL errors come from.
...
#>
	...
	$results = $null
    $sql = new-object system.data.sqlclient.sqlconnection("Data Source=tcp:$serverName;Integrated Security=TRUE;Application Name=FSO Powershell Script;Initial Catalog=$databaseName")
	if ($sql.State -ne 'Open') {
		try {
            $sql.Open()
        } catch {
            Write-Host -ForegroundColor Red "Unable to connect to SQL Server $serverName! Exception:`n$_"; return $_
        }
	}
	...
	try {
        ...
        Write-Host -ForegroundColor Red "Query Failed!`n$query`n$_"; return $query,$_
    }
	...
	$sql.Close()
	#if the query or statement returned no records, return a simple "success" return code
	if ($results -eq $null) {
        return 0
    } else {
        $results
...
<#
    .Synopsis
        Returns true if the specified object is of type DBNull, false otherwise.
    .Parameter Obj
...
#>
    param(
    [parameter(Mandatory=$true,ValueFromPipeline=$true)]$ObjProperty
    )
    process {
        $objProperty.GetType().Name -eq 'DBNull'
    }
}
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUQny8/NgtWlORfKRw89BuEdbz
# RHeggg+4MIID7jCCA1egAwIBAgIQfpPr+3zGTlnqS5p31Ab8OzANBgkqhkiG9w0B
...
# MRYEFKYZhcznHyn7BAW5ETGBJ1PHsB49MA0GCSqGSIb3DQEBAQUABIIBADfwDSlH
# naa/J0xjTm9TMVkGixZz2u8Yj6qW91rKYJ/M35vWTqXOk7//qMXQ4OQR4GSpAzWE
# 8UVV/E/XJScyY5Tsp3qrRC/qOXzLmcsmueFopXXY4eRK30y37zxTcjHEt+TmxOt2
# 4LKGz+JyaiY522DMS3bthsNwqktgyDMsMYkaULoIpTyIDD2bE2jBK/iwWjq9w7Qd
# +iGJDru5nAkQ2OhgK/K+RLTGxHTppJ8nkRegyTTAzwUXllyV/nyLQWzJg9TeDJuA
# uV83A1KkIGw0G/LcNylDefDOfboo6dm6eXbs8X9jDNfF969dmsKl0qU0E5RElSm3
# yUwTWAJjuWoZxoWhggILMIICBwYJKoZIhvcNAQkGMYIB+DCCAfQCAQEwcjBeMQsw
...
# DQEHATAcBgkqhkiG9w0BCQUxDxcNMTMwNjA1MDEwMzQwWjAjBgkqhkiG9w0BCQQx
# FgQU+SgHneq//2jUMSuO3Hd1o9BbvjUwDQYJKoZIhvcNAQEBBQAEggEAgoiZNlgg
# mc9h56JKWQ5bdAn7GzTs45YQPIeFAe2FRTTu6xFhS9P0DWnhnK1px8GqpCwe/DUi
# sNvb8IPss78jWh9jLaiZIzRBTXWebLMqNub23+FWW6m1AVv+wJbaKR0z2xoweSnz
# zsKI3QvE/BxqL//GMM5llhf9YJkeftetVodfftOAkNY7Z56LXBS61PSkqT4FWk1R
# tri7y8z9S+jMJJgFTXqLKBQ9wrtAdm6unJVF3ZHJFx6gxmZeu7aML/xG/+xHZr42
# UFdlPMO/RSo6+PQmQTm2R4XLUzZD+o8vmXyhAYSpmQg4yUvFUVr5VESOC5wXaGk3
# +CS+yDQKFUovBQ==
